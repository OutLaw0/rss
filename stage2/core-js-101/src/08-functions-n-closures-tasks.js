...
function getPowerFunction(exponent) {
  return (x) => {
    if (exponent < 1) return Math.sqrt(x);
    return x ** exponent;
  };
}
...
function getPolynom(...args) {
  ...
  function wrapper(x) {
    ...
    const result = (args[2] * (x ** args[2]) || 0) + (args[1] * x || 0) + (args[0]);
    return result;
  }
  return wrapper;
}
...
function retry(func, attempts) {
  ...
  return function catcher() {
    try {
      return func.call(retry);
    } catch (err) {
      if (cache < attempts) { cache += 1; catcher(); }
    }
    return func.call(retry);
  };
}
...
function logger(func, logFunc) {
  function wrapper(...args) {
    const logging = JSON.stringify(args).slice(1, -1);
    logFunc.call(null, `${func.name}(${logging}) starts`);
    const result = func.call(null, ...args);
    logFunc.call(null, `${func.name}(${logging}) ends`);
    return result;
  }
